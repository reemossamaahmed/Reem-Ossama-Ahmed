...
class Product extends Model implements ConnectTo
{
    ...
    {
        $query = "SELECT * FROM ".self::table." WHERE `status` = ? ORDER BY `price` ASC, `name_en` ASC";
        $stmt = $this->conn->prepare($query);
        $stmt->bind_param('i',$status);
        $stmt->execute();
        return $stmt;
    }
    public function getProductsBySubcategoryId($status = 1)
    {
        $query = "SELECT * FROM ".self::table." WHERE `status` = ? AND `subcategory_id` = ? ORDER BY `price` ASC, `name_en` ASC";
        $stmt = $this->conn->prepare($query);
        $stmt->bind_param('ii', $status , $this->subcategory_id);
        $stmt->execute();
        return $stmt;
    }
    public function getProductsByBrandId($status = 1)
    {
        $query = "SELECT * FROM ".self::table." WHERE `status` = ? AND `brand_id` = ? ORDER BY `price` ASC, `name_en` ASC";
        $stmt = $this->conn->prepare($query);
        $stmt->bind_param('ii',$status,$this->brand_id);
        $stmt->execute();
        return $stmt;
    }
    public function checkSubctegoryIdExistOrNot()
    {
        $query = "SELECT `id` , `subcategory_id` FROM " .self::table ." WHERE `subcategory_id` = ? ";
        $stmt = $this->conn->prepare($query);
        $stmt->bind_param('i',$this->subcategory_id);
        $stmt->execute();
        return $stmt;
    }
    public function checkBrandIdExistOrNot()
    {
        $query = "SELECT `id` , `brand_id` FROM " .self::table ." WHERE `brand_id` = ? ";
        $stmt = $this->conn->prepare($query);
        $stmt->bind_param('i',$this->brand_id);
        $stmt->execute();
        return $stmt;
    }
    public function getOneProductById()
    {
