...
$pro = $products->getAllProducts()->get_result();
...
if($_GET)
{
    ...
    {
        if(is_numeric($_GET['id_subcategory']))
        {
                $products->setSubcategory_id($_GET['id_subcategory']);
                if($products->checkSubctegoryIdExistOrNot()->get_result()->num_rows >= 1)
                {
                    $pro = $products->getProductsBySubcategoryId()->get_result();
                }
                else
                {
    ...
    {
        if(is_numeric($_GET['id_brand']))
        {
                $products->setBrand_id($_GET['id_brand']);
                if($products->checkBrandIdExistOrNot()->get_result()->num_rows >= 1)
                {
                    $pro = $products->getProductsByBrandId()->get_result();
                }
                else
                {
                    include "layouts/errors/404.php";die;        
    ...
    elseif(isset($_GET['id_category']))
    {
        ...
        {
            $categories->setId($_GET['id_category']);
                if($categories->checkCategoryIdExistOrNot()->get_result()->num_rows >= 1)
                {
                    $pro = $categories->getProductsByCategoryId()->get_result();
                }
                else
                {
                    include "layouts/errors/404.php";die;        
                ...
                <div class="grid-list-product-wrapper">
                    <div class="product-grid product-view pb-20">
                        <div class="row">
                            <?php
                            if ($pro->num_rows >= 1) {
                                foreach ($pro->fetch_all(MYSQLI_ASSOC) as $product) { ?>
